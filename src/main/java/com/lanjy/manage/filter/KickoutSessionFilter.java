package com.lanjy.manage.filter;
...
public class KickoutSessionFilter extends AccessControlFilter {
	...
	public static void out(ServletResponse response, ResponseResult result){
		...
		try {
			...
			logger.error("用户在线数量限制【lanjy-manager-->KickoutSessionFilter.out】响应json信息成功");
		} catch (Exception e) {
			logger.error("用户在线数量限制【lanjy-manager-->KickoutSessionFilter.out】响应json信息出错", e);
		}finally{
			if(null != out){
				out.flush();
				out.close();
